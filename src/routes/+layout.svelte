<script lang="ts">
	...
	const nav = [
		...
		{ href: '/keys', label: 'Keys', icon: 'shield' },
	...
	>
		...
		<div class="flex items-center gap-2.5 border-b px-4 py-3.5" style="border-color: var(--border-subtle);">
			...
			<div class="min-w-0">
				...
				<p class="truncate text-[0.625rem] uppercase tracking-wider" style="color: var(--text-muted);">{data.vaultTitle}</p>
			</div>
		</div>
		...
		>
			...
			<div class="ml-auto flex items-center gap-2">
				{#if data.vaults.length > 0}
					<form action="/api/select-vault" method="POST" class="flex items-center">
						<select
							name="vault"
						...
						>
							{#each data.vaults as vault}
								<option
									value={vault.name}
									selected={vault.name === data.selectedVault}
								>
